Scriptname DLC01:DLC01_SimpleElevatorMasterScript extends ObjectReference Hidden
{Same script as SimpleElevatorMasterScript, except user can now define an animation to wait for, for both doors}
...
Group Required_Properties
	...
	Objectreference Property CallButtonTop Auto
	Objectreference Property CallButtonBottom Auto
...
Group Optional_Properties
	...
	String Property AnimToWaitOnLink07ToOpenDoor = "Done" Auto
	String Property AnimToWaitOnLink07ToCloseDoor = "Done" Auto
	String Property AnimToWaitOnLink08ToOpenDoor = "Done" Auto
	String Property AnimToWaitOnLink08ToCloseDoor = "Done" Auto
...
auto STATE WaitingForActivate
	Event ObjectReference.onActivate(objectReference akSender, ObjectReference akActionRef)
		...
		if(akSender == CallButtonTop && bElevatorAtTop) || (akSender == CallButtonBottom && !bElevatorAtTop)
			;means player has activated a call button with the platform already near the player
			;if the elevator is at the top and there's a top door, then open it
			if TopDoorReference && bElevatorAtTop
		    	TopDoorReference.PlayAnimationAndWait(AnimToPlayOnLink08ToOpenDoor, AnimToWaitOnLink08ToOpenDoor)
		    else
		    	;if the elevator is at the bottom and there's a bottom door, then open it
				if BottomDoorReference && !bElevatorAtTop
		    		BottomDoorReference.PlayAnimationAndWait(AnimToPlayOnLink07ToOpenDoor, AnimToWaitOnLink07ToOpenDoor)
		    	endif
		    endif
			;don't do anything else because we don't want the player to have to wait  for the platform to come all the way back
		else

			if BottomDoorReference && !bElevatorAtTop
		    	BottomDoorReference.PlayAnimationAndWait(AnimToPlayOnLink07ToCloseDoor, AnimToWaitOnLink07ToCloseDoor)
		    endif

		    	; If there is a top door, and the elevator is at the top, then wait and open the door
			if TopDoorReference && bElevatorAtTop
		    	TopDoorReference.PlayAnimationAndWait(AnimToPlayOnLink08ToCloseDoor, AnimToWaitOnLink08ToCloseDoor)
		    endif
...
Function SimpleElevatorSetupFunction()
	    ...
	    ;register for onActivate for the top and bottom call buttons
